...
var gabenscoin = require("node-gabencoin")()
...
// Connect to gabenscoin daemon
gabenscoin.auth(credentials.rpc.username, credentials.rpc.password);

// Steam GroupID
var gabensTipGroupID: string = "103582791435206024";

var donationAddress: string = "7mSB8PRUysz7kuc1jNKaDAAEf16zVKQ8sC";
...
// Connect to MongoDB
MongoClient.connect("mongodb://localhost:27017/gabenscointipbotdb", function(err: any, db: mongodb.Db) {
if (err)
	throw err
var Collections: {
...
bot.on("loggedOn", function(): void {
	...
	bot.joinChat(gabensTipGroupID);
	//bot.sendMessage(gabensTipGroupID, "gabenstippingbot is back online");
...
function getPrices(): void {
	async.parallel([
		...
		function(callback) {
			// Mintpal gbn/BTC price
			getHTTPPage("https://api.comkort.com/v1/public/market/summary?market_alias=gbn_btc", callback);
		}
		...
		try {
			...
			prices["gbn/BTC"] = parseFloat(JSON.parse(results[1]).markets["gbn/BTC"].last_price);
		}
		catch(e) {
			return;
		}
		prices["gbn/USD"] = prices["BTC/USD"] * prices["gbn/BTC"];
...
function meCommand(chatterID: string, message: string, group: boolean = true) {
	var toSend: string = (group) ? gabensTipGroupID : chatterID;
	...
	async.parallel([
		...
		var statsMessage: string = 
			[
				...
				"Total tip volume: " + totalAmount + " gabenscoins"
			].join("\n");
...
function priceCommand(chatterID: string, message: string, group: boolean = true) {
	...
	var priceMessage: string[] = [
		...
		"gbn/BTC: " + prices["gbn/BTC"].toFixed(8) + " BTC (https://comkort.com/market/trade/gbn_btc)",
		"gbn/USD: $" + prices["gbn/USD"].toFixed(8),
		"1 gbn = 1 gbn"
	];
	if (message.split(" ")[1]) {
		var amount: number = numeral().unformat(message.split(" ")[1]);
		var amountUSD: number = amount * prices["gbn/USD"];
		priceMessage.push(amount + " gabenscoins = " + numeral(amountUSD).format("$0,0.00"));
	}
...
function inviteToGroup(invitee) {
	botWebLogOn(function(steamCookies: string[]): void {
		...
		requester.post({url: "http://steamcommunity.com/actions/GroupInvite", jar: j, form: {
			...
			"inviter": bot.steamID,
			"invitee": invitee,
			"group": gabensTipGroupID, // gabenscoin group
...
bot.on("chatMsg", function(sourceID: string, message: string, type: number, chatterID: string): void {
	if (message[0] === "+") {
    	switch (message.split(" ")[0]) {
			...
			case "+joingame":
			case "+joinserver":
				bot.sendMessage(gabensTipGroupID, "Our TF2 server is at steam://connect/80.240.134.134:27015/ Click to join and place wagers on matches!");
				break;
			default:
				bot.sendMessage(gabensTipGroupID, "I won't respond to commands in the group chat. Open up a private message by double clicking on my name in the sidebar to send me commands.");
    	}
  	}
});
	...
	switch (message.split(" ")[0]) { // The command part
		case "+register":
			...
			Collections.Users.findOne({"id": chatterID}, function(err: Error, previousUser) {
				...
				gabenscoin.getNewAddress(chatterID, function(err: Error, address: string) { // chatterID is that user's account
					if (err) {
					...
					Collections.Users.insert(userEntry, {w:1}, function(err: Error) {
						...
						bot.sendMessage(chatterID, "Tip users with '+tip <STEAM NAME> <AMOUNT> gabenscoins'");
						...
						Collections.OldUsers.findOne({"id": chatterID}, function(err: Error, oldUser) {
							...
							if (oldUser && oldUser.funds > 0) {
								...
								var tipComment = {
									"sender": "gabenstippingbot (v1)",
									"recipient": name,
									"refund": false,
									"USD": oldUser.funds * prices["gbn/USD"]
								};
								gabenscoin.move("OldUsersPool", chatterID, oldUser.funds, 1, stringifyAndEscape(tipComment), function(err: any, success: boolean) {
									if (err) {
										bot.sendMessage(chatterID, reportError(err, "Moving balance for old user"));
										return;
									}
									bot.sendMessage(chatterID, "Hey " + name + ", " + oldUser.funds + " gabenscoins has been added to your account from your v1 wallet.");
								});
							}
						});
						// Check for giveaway
						...
						if (giveawayInfo.happening) {
							...
							var tipComment = {
								"sender": "gabenstippingbot",
								"recipient": name,
								"refund": false,
								"USD": amountToGive * prices["gbn/USD"]
							};
							gabenscoin.move(giveawayInfo.account, chatterID, amountToGive, 1, stringifyAndEscape(tipComment), function(err: any, success: boolean) {
								if (err) {
									bot.sendMessage(chatterID, reportError(err, "Moving balance for a giveaway"));
									return;
								}
								bot.sendMessage(chatterID, "As part of the current giveaway, you've been given " + amountToGive + " gabenscoin! You can use that gabenscoins to tip others on Steam and help spread the word!");
							});
						}
					});
				});
		...
		case "+balance":
			Collections.Users.findOne({"id": chatterID}, function(err: Error, user) {
				...
				gabenscoin.getBalance(chatterID, function(err: Error, balance: number) {
					if (err) {
						bot.sendMessage(chatterID, reportError(err, "Retrieving user balance in +balance"));
						return;
					}
					bot.sendMessage(chatterID, "Your current balance is: " + balance + " gabenscoins");
					bot.sendMessage(chatterID, "Your deposit address is: " + user.address);
				});
			});
			break;
		case "+history":
			Collections.Users.findOne({"id": chatterID}, function(err: Error, user) {
				...
				async.parallel([
					function(callback) {
						gabenscoin.getBalance(chatterID, callback);
					},
					function(callback) {
						// Get 20 most recent transactions because moves from the FeePool also count and must be expunged
						gabenscoin.listTransactions(chatterID, numberOfTransactions * 2, callback);
					}
					...
					message += "Your current balance is: " + balance + " gabenscoins\n";
					...
					for (var i: number = 0; i < transactions.length; i++) {
						...
						var time: Date = new Date(transaction.time * 1000); // gabenscoind returns a time that is missing the last 3 digits so multiplying by 1000 fixes this
		...
		case "+withdraw":
			Collections.Users.findOne({"id": chatterID}, function(err: Error, user) {
				...
				if (!user) {
					bot.sendMessage(chatterID, "You must be registered to withdraw your gabenscoins");
					return;
				}
				gabenscoin.getBalance(chatterID, function(err: Error, balance: number) {
					if (err) {
					...
					if (sendToAddress === undefined) {
						bot.sendMessage(chatterID, "Missing address. Notation for +withdraw is '+withdraw <ADDRESS> <AMOUNT|all> gabenscoins'.");
						return;
					}
					...
					if (rawAmount === undefined) {
						bot.sendMessage(chatterID, "Missing amount. Notation for +withdraw is '+withdraw <ADDRESS> <AMOUNT|all> gabenscoins'.");
						return;
					}
					...
					if (sendAmount < 1) {
						bot.sendMessage(chatterID, "Invalid amount of gabenscoins to withdraw");
						return;
					}

					gabenscoin.sendFrom(chatterID, sendToAddress, sendAmount, function(err: any, txid: string) {
						if (err) {
							if (err.code === -5) {
								...
								bot.sendMessage(chatterID, "You have insufficient funds to withdraw " + sendAmount + " gabenscoins");
								bot.sendMessage(chatterID, "Your current balance is: " + balance + " gabenscoins");
							}
							else {
						...
						bot.sendMessage(chatterID, "Sent " + sendAmount + " gabenscoins to " + sendToAddress + " in tx " + txid);
						// Reimburse the user for their transaction fee
						gabenscoin.getTransaction(txid, function(err: any, txInfo: any) {
							if (err) {
							...
							if (fee === 0) {
								bot.sendMessage(chatterID, "The transaction fee was 0 gabenscoins");
								return;
							}
							gabenscoin.move("FeePool", chatterID, fee, function(err: any, success: boolean) {
								if (err) {
									bot.sendMessage(chatterID, reportError(err, "Reimbursing the user for their transaction fee"));
									return;
								}
								bot.sendMessage(chatterID, "The transaction fee of " + fee + " gabenscoins has been reimbursed");
							});
						});
					});
				});
		...
		case "+help":
			var helpMessage: string = 
				[
					...
					"New to gabenscoin? Visit the official page: http://www.gabenscoin.net",
					"",
					"Commands:",
					...
					"	+balance - Check the amount of gabenscoins in your account",
					"	+history - Display your current balance and a list of your 10 most recent transactions",
					"	+withdraw <ADDRESS> <AMOUNT|all> gabenscoins - Withdraw funds in your account to the specified address (the 1 gabenscoins transaction fee will be covered by the bot)",
					"	+tip <STEAM NAME|COMMUNITY URL> <AMOUNT|all> gabenscoins [+verify] - Send a Steam user a tip. To send tips to users that aren't registered with the bot, tip to their profile URL. (More details about tipping available at http://steamdogebot.com/ ) If +verify is added, the bot will send a message confirming the tip to the group chat.",
					"	+donate <AMOUNT|all> gabenscoins - Donate gabenscoins to the developer to keep the bot alive. The server costs about 17,000 gabenscoins a month. Any help is greatly appreciated!",
			...
			break;
		case "+version":
			bot.sendMessage(chatterID, "gabenscoinTippingBot " + version + " by Ryan Petschek (RazeTheRoof) <petschekr@gmail.com>\nDonate to " + donationAddress + " if you enjoy this bot and want keep it running. Servers cost money! (You can also send the bot '+donate <AMOUNT> gabenscoin' to donate from your tipping balance.)");
			break;
		case "+donate":
			Collections.Users.findOne({"id": chatterID}, function(err: Error, user) {
				if (err) {
				...
				if (!user) {
					bot.sendMessage(chatterID, "You must be registered to donate with your tipping account. You can also send some gabenscoins over to " + donationAddress);
					return;
				}
				gabenscoin.getBalance(chatterID, function(err: any, balance: number) {
					if (err) {
					...
					if (rawDonationAmount === undefined) {
						bot.sendMessage(chatterID, "Missing amount. Notation for +donate is '+donate <AMOUNT|all> gabenscoins'.");
						return;
					}
					...
					if (donationAmount < 1) {
						bot.sendMessage(chatterID, "Invalid amount of gabenscoins to donate");
						return;
					}
					gabenscoin.sendFrom(chatterID, donationAddress, donationAmount, function(err: any, txid: string) {
						if (err) {
							if (err.code === -4) {
								// Wallet probably doesn't have enough funds
								...
								bot.sendMessage(chatterID, "You have insufficient funds to donate " + donationAmount + " gabenscoins");
								bot.sendMessage(chatterID, "Your current balance is: " + balance + " gabenscoins");
							}
							else {
						...
						bot.sendMessage(chatterID, "Your donation of " + donationAmount + " gabenscoins was successfully donated. (Donation address: " + donationAddress + ")\nTxID for this donation is: " + txid + "\nThank you very much for your support of this project.");
						...
						Collections.Donations.insert({
							...
							"USD": donationAmount * prices["gbn/USD"],
							"timestamp": Date.now(),
							"time": new Date().toString(),
							"groupID": gabensTipGroupID,
		...
		case "+tip":
			Collections.Users.findOne({"id": chatterID}, function(err: Error, user) {
				...
				gabenscoin.getBalance(chatterID, function(err: any, balance: number) {
					...
					if (tipInfo) {
						...
						bot.sendMessage(chatterID, "Invalid +tip format. Notation for +tip is '+tip <STEAM NAME|COMMUNITY URL> <AMOUNT|all> gabenscoins'.");
						return;
					}
					var amount: number = 0;
					if (rawAmount.toLowerCase() === "all") {
						...
						if (amount === 0) {
							bot.sendMessage(chatterID, "Invalid gabenscoins amount to tip entered");
							return;
						}
					}
					if (amount > balance) {
						bot.sendMessage(chatterID, "Insufficient funds to tip " + amount + " gabenscoins");
						bot.sendMessage(chatterID, "You can deposit more gabenscoins to your deposit address: " + user.address);
						return;
					}
					if (amount < 1) {
						bot.sendMessage(chatterID, "You must tip at least 1 gabenscoins");
						return;
					}
							...
							Collections.Users.findOne({"id": personToTipID}, function(err: Error, personToTip): void {
								...
								if (personToTip) {
									...
									Collections.Blacklist.findOne({"id": personToTipID}, function(err: Error, blacklistedUser) {
										...
										gabenscoin.getNewAddress(personToTipID, function(err: Error, address: string) {
											if (err) {
						...
						Collections.Users.find({name: personToTipName}).toArray(function(err: Error, possibleUsers: any[]) {
							...
							if (personToTipID === undefined) {
								if (possibleUsers.length < 1) {
									...
									bot.sendMessage(chatterID, "Then, tip with '+tip <COMMUNITY URL> <AMOUNT> gabenscoin'");
									bot.sendMessage(chatterID, "For example, '+tip http://steamcommunity.com/id/razed/ 100 gabenscoin +verify'");
								...
								if (possibleUsers.length > 1) {
									...
									bot.sendMessage(chatterID, "Then, tip with '+tip <COMMUNITY URL> <AMOUNT> gabenscoins'");
									bot.sendMessage(chatterID, "For example, '+tip http://steamcommunity.com/id/razed/ 100 gabenscoin +verify'");
							...
							if (personToTipID === bot.steamID) {
								bot.sendMessage(chatterID, "I'm sorry, but you can't tip me. If you would like to donate, please reply with '+donate <AMOUNT> gabenscoins'. Thank you!");
								return;
							}
							var tipComment = {
								"sender": user.name,
								...
								"refund": false,
								"USD": amount * prices["gbn/USD"]
							};
							gabenscoin.move(chatterID, personToTipID, amount, 1, stringifyAndEscape(tipComment), function(err: any, success: boolean) {
								if (err) {
								...
								if (/\+verify/i.test(message))
									bot.sendMessage(gabensTipGroupID, personToTipName + " was tipped " + amount + " gabenscoins by " + user.name + "!");
								...
								Collections.Tips.insert({
									...
									if (!unregisteredUser) {
										...
										bot.sendMessage(chatterID, "You tipped " + personToTipName + " " + amount + " gabenscoins successfully");
										bot.sendMessage(personToTipID, "You were tipped " + amount + " gabenscoins by " + user.name);
									}
									else {
										bot.sendMessage(chatterID, "You tipped " + personToTipName + " " + amount + " gabenscoins successfully. If they do not accept the tip within " + purgeTime + " hours, the tip will be refunded back to you.");
									}
								});
							});
						});
			...
			break;
		case "+accept":
			// Accept a pending tip and welcome that user to gabenscoin
			// Unfriend the user
			...
			Collections.Tips.findOne({"recipient.id": chatterID, "unregisteredUser": true}, function(err: Error, tip: any) {
				...
				async.parallel([
					...
					bot.sendMessage(chatterID, "Congrats, your tip of " + tip.amount + " gabenscoins from " + tip.sender.name + " was accepted! Welcome to gabenscoin next generation Trade community");
...
bot.on("friend", function(steamID: string, relationship: number): void {
	...
	if (relationship === Steam.EFriendRelationship.RequestRecipient) {
		...
		setTimeout(function(): void {
			bot.sendMessage(steamID, "Go to the gabensTradeBase Tip group to message me. I can't accept friend requests.");
...
bot.on("user", function(userInfo): void {
	Collections.Users.findOne({"id": userInfo.friendid}, function(err: Error, user) {
		...
		Collections.Tips.findOne({"recipient.id": userInfo.friendid, "accepted": false, "refunded": false}, function(err: Error, tip: any) {
			if (err) {
				bot.sendMessage(gabensTipGroupID, reportError(err, "Retrieving tip in friend accept handler"));
				return;
			}
			if (!user || !tip)
				return;
			bot.sendMessage(userInfo.friendid, "Hello " + tip.recipient.name + ", you've been tipped " + tip.amount + " gabenscoins by " + tip.sender.name);
			bot.sendMessage(userInfo.friendid, "gabenscoin is a 1st Revolutionary digital Gaming currency sent through the internet. You can find out more at http://gabens-coin.org/");
			bot.sendMessage(userInfo.friendid, "This is your last chance. After this, there is no turning back. You take the Paper Money - the story ends, you wake up in your bed and believe whatever you want to believe.You take the gabenscoin Train - you stay in Wonderland and I show you how deep the rabbit-hole goes.");			
		...
		async.each(unregisteredUserTips, function(tip, callback) {
			...
			if ((currentTime - tipTime) > (60 * 60 * purgeTime * 1000)) { // x hours in milliseconds
				...
				var tipComment = {
					...
					"refund": true,
					"USD": tip.amount * prices["gbn/USD"]
				};
				gabenscoin.move(tip.recipient.id, tip.sender.id, tip.amount, 1, stringifyAndEscape(tipComment), function(err: any, success: boolean) {
					if (err) {
						callback(err);
						return;
					}
